...
Bounce::Game::Game(EngineContext& context) :
    Scene(context),
...
void Game::EventBallsMissed(int ballsMissed)
{
    ...
    if (stats.lives <= 0)
    {
        ...
        LOG_INFO("Game Over! Score: {}", stats.score);
...
{
    int ballsMissed = (int)std::erase_if(
        balls, [](const Ball& ball) { return IsOutsideWindowBottom(ball.shape); });
