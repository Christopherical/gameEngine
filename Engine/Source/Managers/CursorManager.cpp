    ...
    visible_(true)
{
    ...
    shape_.setPosition(gConfig.windowSize / 2.0f);
}
...
void CursorManager::Update(float deltaTime)
{
    ...
    if (joystickDirection.length() > gConfig.joystickDeadzone)
    {
        ...
        shape_.setPosition({std::clamp(shape_.getPosition().x, 0.0f, gConfig.windowSize.x - 1),
                            std::clamp(shape_.getPosition().y, 0.0f, gConfig.windowSize.y - 1)});
